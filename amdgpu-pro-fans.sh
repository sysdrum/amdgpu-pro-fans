...
usage ()
{
	...
	echo "usage: $0 [-a <0,1,2,3...> to set card number]"
	...
	if [ -d $CurrentCard ]; then
		for CurrentMonitor in "$CurrentCard"device/hwmon/hwmon?/ ; do
			...
			if [ $fanmax -gt 0 ] ; then	
				...
				if [ $(( speedresults - speed )) -gt 6 ] ; then
					if [ "$verbosity" -eq 0 ] ; then echo "-2" ; fi
					if [ "$verbosity" -eq 1 ] ; then echo "Error Setting Speed For Card$adapter!" ; fi 
				else
					if [ "$verbosity" -eq 0 ] ; then echo "$fanpercent" ; fi
					if [ "$verbosity" -eq 1 ] ; then echo "Card$adapter Speed Set To $fanpercent %" ; fi 
				fi
			else
				if [ "$verbosity" -eq 0 ] ; then echo "-1" ; fi
				if [ "$verbosity" -eq 1 ] ; then echo "Error: Unable To Determine Maximum Fan Speed For Card$adapter!" ; fi
			fi
		done
	else
		if [ "$verbosity" -eq 0 ] ; then echo "-3" ; fi
		if [ "$verbosity" -eq 1 ] ; then echo "Error: Unable to find the specified card" ; fi
	fi
}
